...
a quantity justified by Lemma 8 below, where the exponent $-7$ rather than $-8$ pays for sampling a permutation without replacement. Six hypotheses are used, and none of them is free. They form the antecedent of every theorem below, so it is worth saying plainly what assuming them does and does not amount to. None is proved here, and two of them cannot be proved by anyone.

(H4) and (H5) are the standard cryptographic assumptions. No proof of either is known, and the asymptotic form of such a statement would establish that one-way functions exist, hence that $P \neq NP$. They are therefore treated not as true or false but as parameters: the advantages $\epsilon_{\mathrm{prf}}$ and $\epsilon_{\mathrm{prp}}$ are carried through every bound and appear in the success probability of Theorem 7, and $\epsilon_{\mathrm{prp}}$ sits inside $\rho$ above. If the primitives are weak the theorems below remain true and become vacuous, which is the behaviour one wants from a quantitative statement.

The other four are empirical claims about a deployment rather than mathematical ones. (H2) is a measurement of a compiled binary on a particular processor, (H3) a claim about its branch structure, (H1) a condition on the network together with a constraint on how the adversary schedules its injections, and (H6) a statement of what it has calibrated in advance. These are checked against a target, not derived. Where the protocol does force something about them it is proved: (H1) itself yields $s = \Omega(c_H)$, and Remark 2 turns that into the unconditional floor $r > 0.107$. No upper bound on $r$ is claimed anywhere, because that one is governed by jitter rather than by the protocol.
...
Naming the six separately is what makes failure localisable. If (H2) fails there is no gap and nothing below applies; if (H1) fails no sample size suffices; if (H4) or (H5) fails the bounds survive but carry no content. If (H3) fails the receiver is a different machine from the one analysed, and this is the hypothesis the deployed countermeasure attacks: decrypting in constant time makes the timing profile independent of the parse, and falsifies (H3) by construction. The list is therefore also a specification of the defence.
